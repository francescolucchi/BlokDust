...
define(["require", "exports", '../Device', './../Blocks/Sources/ToneSource', './../Blocks/Interaction/ComputerKeyboard', './../Blocks/Effects/Post/Chopper', './../Blocks/Power/Power', './../Blocks/Power/ParticleEmitter'], function (require, exports, Device_1, ToneSource_1, ComputerKeyboard_1, Chopper_1, Power_1, ParticleEmitter_1) {
    ...
    var Tutorial = (function (_super) {
        ...
        Tutorial.prototype.Init = function (drawTo) {
        ...
        Tutorial.prototype.CheckLaunch = function () {
            if (this.Debug) {
                this.OpenSplash();
            }
            else {
        ...
        //-------------------------------------------------------------------------------------------
        Tutorial.prototype.CheckTask = function () {
            if (this.Open) {
                var check = false;
                this.WatchedBlocks = [];
                ...
                for (i = 0; i < App.Blocks.length; i++) {
                    if (App.Blocks[i] instanceof ToneSource_1.ToneSource && !tone) {
                        tone = App.Blocks[i];
                        this.WatchedBlocks[0] = tone;
                    }
                    if (App.Blocks[i] instanceof ComputerKeyboard_1.ComputerKeyboard && !controllerUnplugged) {
                        controllerUnplugged = App.Blocks[i];
                        this.WatchedBlocks[1] = controllerUnplugged;
                    }
                    if (App.Blocks[i] instanceof Chopper_1.Chopper && !effectUnplugged) {
                        effectUnplugged = App.Blocks[i];
                        this.WatchedBlocks[4] = effectUnplugged;
                    }
                    if (App.Blocks[i] instanceof ParticleEmitter_1.ParticleEmitter && !particle) {
                        particle = App.Blocks[i];
                        this.WatchedBlocks[5] = particle;
                    }
                    if (App.Blocks[i] instanceof Power_1.Power && !power) {
                        power = App.Blocks[i];
                ...
                if (tone && tone.Connections.Count) {
                    ...
                    for (i = 0; i < toneConnections.length; i++) {
                        if (toneConnections[i] instanceof ComputerKeyboard_1.ComputerKeyboard && !controller) {
                            controller = toneConnections[i];
                            this.WatchedBlocks[2] = controller;
                        }
                        if (toneConnections[i] instanceof Chopper_1.Chopper && !effect) {
                ...
                if (power && power.Connections.Count) {
                    ...
                    for (i = 0; i < powerConnections.length; i++) {
                        if (powerConnections[i] instanceof ParticleEmitter_1.ParticleEmitter && !particleConnected) {
                ...
                if (this._TutKeys) {
                    switch (this.CurrentScene) {
                        case 1:
                            if (tone && !App.MainScene.IsDraggingABlock) {
                                check = true;
                                App.MainScene.MainSceneDragger.Jump(tone.Position, new Point(cx, 0.5), 1000);
                            }
                            break;
                        case 2:
                            if (controller && !App.MainScene.IsDraggingABlock) {
                                check = true;
                            }
                            break;
                        case 3:
                            if (effect && !App.MainScene.IsDraggingABlock) {
                                check = true;
                            }
                            break;
                        case 4:
                            if (this.OptionsInteract && App.MainScene.SelectedBlock == effect) {
                                check = true;
                                App.MainScene.MainSceneDragger.Jump(tone.Position, new Point(cx, 0.5), 1000);
                            }
                            break;
                        case 5:
                            if (!controller && !controllerUnplugged && !App.MainScene.IsDraggingABlock) {
                                check = true;
                                App.MainScene.MainSceneDragger.Jump(tone.Position, new Point(cx, 0.35), 1000);
                            }
                            break;
                        case 6:
                            if (particle && !App.MainScene.IsDraggingABlock) {
                                check = true;
                            }
                            break;
                        case 7:
                            if (particleConnected && !App.MainScene.IsDraggingABlock) {
                                check = true;
                            }
                            break;
                        default:
        ...
        Tutorial.prototype.AnimateButton = function () {
        ...
        Tutorial.prototype.WordWrap = function (context, text, x, y, lineHeight, fitWidth) {
            fitWidth = fitWidth || 0;
            if (fitWidth <= 0) {
        ...
        Tutorial.prototype.LineCount = function (context, text, fitWidth) {
            fitWidth = fitWidth || 0;
            if (fitWidth <= 0) {
                return 1;
            }
        ...
        Tutorial.prototype.NumberWithCommas = function (x) {
            return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        };
        //-------------------------------------------------------------------------------------------
        //  TWEEN
        //-------------------------------------------------------------------------------------------
        Tutorial.prototype.DelayTo = function (panel, destination, t, delay, v) {
        ...
        //  INTERACTION
        //-------------------------------------------------------------------------------------------
        Tutorial.prototype.OpenSplash = function () {
        ...
        Tutorial.prototype.CloseSplash = function () {
            this.SplashOpen = false;
            this.DelayTo(this.Offset2, -1, 0.6, 0, "x");
        };
        Tutorial.prototype.OpenPanel = function () {
            if (App.Blocks.length) {
        ...
        Tutorial.prototype.ClosePanel = function () {
            this.Open = false;
            App.MainScene.TutorialHotspots.Points = [];
            this.WatchedBlocks = [];
            this.DelayTo(this.Offset, -20, 0.5, 0, "x");
        };
        Tutorial.prototype.NextScene = function () {
            var t = 0.4;
        ...
        Tutorial.prototype.UpdateHotspots = function () {
            var ctx = this.Ctx;
            var units = App.Unit;
        ...
        Tutorial.prototype.GutterCheck = function (n, w, cat, units) {
            if (cat.Pages === 0) {
        ...
        Tutorial.prototype.SplashMouseDown = function (point) {
            this.HitTests(point);
            if (this.SplashOpen) {
                if (this._RollOvers[0]) {
                    this.OpenPanel();
        ...
        Tutorial.prototype.MouseDown = function (point) {
            this.HitTests(point);
            if (this._RollOvers[2]) {
                this.ClosePanel();
                this.Skip();
        ...
        Tutorial.prototype.MouseUp = function (point) {
        };
        Tutorial.prototype.MouseMove = function (point) {
            this.HitTests(point);
        };
        Tutorial.prototype.HitTests = function (point) {
            var ctx = this.Ctx;
            var units = App.Unit;
            var x = App.Width - (this.Offset.x * units);
            var y = (App.Height * 0.5) + (this.Offset.y * units) - (20 * units);
        ...
        Tutorial.prototype.CountIntroLines = function () {
            var units = App.Unit;
