...
define(["require", "exports", '../Device', './MenuCategory', './Options/OptionColorTheme', './../_Version', './Options/OptionSlider', './Options/OptionMeter', './Options/OptionActionButton'], function (require, exports, Device_1, MenuCategory_1, OptionColorTheme_1, _Version_1, OptionSlider_1, OptionMeter_1, OptionActionButton_1) {
    ...
    var SettingsPanel = (function (_super) {
        ...
        SettingsPanel.prototype.Init = function (drawTo) {
            ...
            this._OpenTab = 0;
            this._VersionNumber = _Version_1.Version;
            //console.log(Version);
            // OPTIONS //
            this.Range = 0;
            this.Margin = 0;
        ...
        SettingsPanel.prototype.Populate = function (json) {
            ...
            var gutter = 60;
            var menuCats = [];
            if (App.Metrics.Device !== Device_1.Device.desktop) {
                gutter = 40;
            }
            ...
            for (var i = 0; i < n; i++) {
                ...
                menuCats[i] = new MenuCategory_1.MenuCategory(point, size, name, offset);
                catX += catWidth[i];
            }
        ...
        //  DRAW
        //-------------------------------------------------------------------------------------------
        SettingsPanel.prototype.Draw = function () {
            var ctx = this.Ctx;
        ...
        SettingsPanel.prototype.WordWrap = function (context, text, x, y, lineHeight, fitWidth) {
            fitWidth = fitWidth || 0;
            if (fitWidth <= 0) {
        ...
        SettingsPanel.prototype.NumberWithCommas = function (x) {
            return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        };
        //-------------------------------------------------------------------------------------------
        //  TWEEN
        //-------------------------------------------------------------------------------------------
        SettingsPanel.prototype.DelayTo = function (panel, destination, t, delay, v) {
        ...
        //  INTERACTION
        //-------------------------------------------------------------------------------------------
        SettingsPanel.prototype.OpenPanel = function () {
            this.Open = true;
            this.OffsetY = -this.DrawTo.Height;
        ...
        SettingsPanel.prototype.ClosePanel = function () {
            this.DelayTo(this, -this.DrawTo.Height, 500, 0, "OffsetY");
        };
        SettingsPanel.prototype.PopulateOptions = function () {
            var optionList = [];
            var optionY = [];
            ...
            // COLOR THEME //
            optionList.push(new OptionColorTheme_1.ThemeSelector(optionNames[0]));
            // VOLUME //
            ...
            optionList.push(new OptionSlider_1.Slider(new Point(sliderX, pageY + this.MenuItems[1].YOffset + (60 * units)), new Size(1, 48 * App.Unit), sliderO, sliderValue, 0, 60, true, optionNames[1], sliderSetting, false));
            // METER //
            optionList.push(new OptionMeter_1.OptionMeter(optionNames[2]));
            // TUTORIAL //
            optionList.push(new OptionActionButton_1.OptionActionButton(new Point(0, pageY + this.MenuItems[1].YOffset + (156 * units)), new Size(1, 48 * App.Unit), optionNames[3], "Launch Tutorial", "tutorial"));
            this.Options = optionList;
        };
        SettingsPanel.prototype.MouseDown = function (point) {
            this.HitTests(point);
        ...
        SettingsPanel.prototype.MouseMove = function (point) {
            this.HitTests(point);
            for (var i = 0; i < this.Options.length; i++) {
        ...
        SettingsPanel.prototype.HitTests = function (point) {
            var units = App.Unit;
            var centerY = this.OffsetY + (this.DrawTo.Height * 0.5);
            var tabY = this.OffsetY;
            var pageY = tabY + (120 * units);
        ...
        SettingsPanel.prototype.MouseUp = function (point) {
            for (var i = 0; i < this.Options.length; i++) {
                this.Options[i].Selected = false;
            }
        };
        // DRAGGING A SLIDER //
        SettingsPanel.prototype.SliderSet = function (n, mx) {
            var units = App.Unit;
            var centerY = this.OffsetY + (this.DrawTo.Height * 0.5);
            var tabY = this.OffsetY;
            var pageY = tabY + (120 * units);
        ...
        SettingsPanel.prototype.ActionButton = function (setting) {
            switch (setting) {
                case "tutorial":
                    this.ClosePanel();
        ...
        SettingsPanel.prototype.UpdateValue = function (object, value, min, max, rangemin, rangemax, setting, axis, log) {
            // CALCULATE VALUE //
        ...
        SettingsPanel.prototype.ShareFacebook = function () {
            var href = "http://www.facebook.com/sharer.php?";
            href = "" + href + "u=http://blokdust.com";
            window.open(href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');
        };
        SettingsPanel.prototype.ShareTwitter = function () {
        ...
        SettingsPanel.prototype.ShareGoogle = function () {
        ...
        //  MATHS
        //-------------------------------------------------------------------------------------------
        SettingsPanel.prototype.ValueInRange = function (value, floor, ceiling) {
            if (value < floor) {
                value = floor;
            }
            if (value > ceiling) {
        ...
        SettingsPanel.prototype.linValue = function (minpos, maxpos, minval, maxval, position) {
            var scale = (maxval - minval) / (maxpos - minpos);
            //console.log("" +minval + " | " +maxval + " | " +position);
            return (position - minpos) * scale + minval;
        };
        SettingsPanel.prototype.linPosition = function (minpos, maxpos, minval, maxval, value) {
            var scale = (maxval - minval) / (maxpos - minpos);
            //console.log("" +minval + " | " +maxval + " | " +value);
            return minpos + (value - minval) / scale;
        };
        SettingsPanel.prototype.Resize = function () {
        };
